...
<body>
	...
	<div class="container-fluid assortiment-container">
		...
		<div class="assortiment-types__item col-lg-4 text-center">ЧЕРНЫЙ ХЛЕБ</div>
		<div class="assortiment-types__item col-lg-4 text-center">ПИРОЖНЫЕ</div>
		<div class="assortiment-types__item col-lg-4 text-center">БЕЛАЯ ВЫПЕЧКА</div></div>
<div class="assortiment-container-types">
	<?php 
	
	
	$son = new mysqli('localhost','root','root', 'zabatonom');
// Возьмите данные профиля из базы данных
 $select = $son->query("SELECT  foto, name, description, ingredient1, ingredient2, ingredient3, value, nal, slogan, t1, t2, t3  FROM ass WHERE types = 'белая выпечка' LIMIT 180");
    while($info = $select->fetch_array()){
    	
    	echo'<div class="wrap-assortiment-container d-flex flex-column col-lg-4">
		<div class="block-content d-flex flex-column col-lg-12"><div class="block-content-admin d-flex col-lg-12 flex-column">
	...
	$select1 = $son->query("SELECT  foto, name, description, ingredient1, ingredient2, ingredient3, value, nal, slogan, t1, t2, t3  FROM ass WHERE types = 'пирожные' LIMIT 180");
    while($info = $select1->fetch_array()){
    	
    	echo'<div class="wrap-assortiment-container d-flex flex-column col-lg-4"><div class="bardcolor fontSegoeScript size20px">'.$info['name'].'</div></div>';
    }?>
